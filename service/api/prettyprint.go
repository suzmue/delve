...
// SinglelineString returns a representation of v on a single line.
func (v *Variable) SinglelineStringTypes(includeTypes bool) string {
	var buf bytes.Buffer
	v.writeTo(&buf, true, false, true, !includeTypes, "", "")
	return buf.String()
}
...
func (v *Variable) writeTo(buf io.Writer, top, newlines, includeType, neverIncludeType bool, indent, fmtstr string) {
	if v.Unreadable != "" {
	...
	if !top && v.Addr == 0 && v.Value == "" {
		if includeType && !neverIncludeType && v.Type != "void" {
	...
	switch v.Kind {
	case reflect.Slice:
		v.writeSliceTo(buf, newlines, includeType, neverIncludeType, indent, fmtstr)
	case reflect.Array:
		v.writeArrayTo(buf, newlines, includeType, neverIncludeType, indent, fmtstr)
	case reflect.Ptr:
		if v.Type == "" || len(v.Children) == 0 {
			...
			if (includeType && !neverIncludeType) || v.Children[0].Addr != 0 {
				fmt.Fprint(buf, "*")
			}
			v.Children[0].writeTo(buf, false, newlines, includeType, neverIncludeType, indent, fmtstr)
		}
	...
	case reflect.Chan:
		if newlines {
			v.writeStructTo(buf, newlines, includeType, neverIncludeType, indent, fmtstr)
		} else {
			if len(v.Children) == 0 {
				fmt.Fprintf(buf, "%s nil", v.Type)
			} else {
	...
	case reflect.Interface:
		...
		if includeType && !neverIncludeType {
	...
	case reflect.Map:
		v.writeMapTo(buf, newlines, includeType, neverIncludeType, indent, fmtstr)
	case reflect.Func:
		if v.Value == "" {
			fmt.Fprint(buf, "nil")
		} else {
...
func (v *Variable) writeSliceTo(buf io.Writer, newlines, includeType, neverIncludeType bool, indent, fmtstr string) {
	if includeType && !neverIncludeType {
	...
	v.writeSliceOrArrayTo(buf, newlines, neverIncludeType, indent, fmtstr)
}

func (v *Variable) writeArrayTo(buf io.Writer, newlines, includeType, neverIncludeType bool, indent, fmtstr string) {
	if includeType && !neverIncludeType {
		fmt.Fprintf(buf, "%s ", v.Type)
	}
	v.writeSliceOrArrayTo(buf, newlines, neverIncludeType, indent, fmtstr)
...
func (v *Variable) writeStructTo(buf io.Writer, newlines, includeType, neverIncludeType bool, indent, fmtstr string) {
	...
	for i := range v.Children {
		...
		v.Children[i].writeTo(buf, false, nl, true, neverIncludeType, indent+indentString, fmtstr)
...
func (v *Variable) writeMapTo(buf io.Writer, newlines, includeType, neverIncludeType bool, indent, fmtstr string) {
	if includeType && !neverIncludeType {
		fmt.Fprintf(buf, "%s ", v.Type)
	}
	if v.Base == 0 && len(v.Children) == 0 {
	...
	for i := 0; i < len(v.Children); i += 2 {
		...
		key.writeTo(buf, false, false, false, neverIncludeType, indent+indentString, fmtstr)
		fmt.Fprint(buf, ": ")
		value.writeTo(buf, false, nl, false, neverIncludeType, indent+indentString, fmtstr)
		if i != len(v.Children)-1 || nl {
			fmt.Fprint(buf, ", ")
		}
	}
...
func (v *Variable) writeSliceOrArrayTo(buf io.Writer, newlines, neverIncludeType bool, indent, fmtstr string) {
	...
	for i := range v.Children {
		...
		v.Children[i].writeTo(buf, false, nl, false, neverIncludeType, indent+indentString, fmtstr)
		if i != len(v.Children)-1 || nl {
			fmt.Fprint(buf, ",")
		}
	}
